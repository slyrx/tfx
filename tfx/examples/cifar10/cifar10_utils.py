...
def _input_fn(filenames, tf_transform_output, batch_size):
  ...
  def transform_fn(raw_features):
    """Function for transform raw features."""
    # transformed_features will contain a dict of transformed tensors which are
    # the result of applying the transform function defined by tf.Transform to
    # the raw features.
    transformed_features = tf_transform_output.transform_raw_features(
        raw_features)
  ...
  dataset = tf.data.experimental.make_batched_features_dataset(
      filenames, batch_size, raw_feature_spec, reader=_gzip_reader_fn)
