...
def start_aip_training(input_dict: Dict[Text, List[types.Artifact]],
  ...
  tfx.scripts.run_executor module on a AIP training job interpreter.
  ...
  Args:
    ...
    training_inputs: Training input for AIP training job. 'pythonModule',
  ...
  # Remove aip_args from exec_properties so AI Platform trainer doesn't call
  # itself
...
def deploy_model_for_aip_prediction(serving_path: Text, model_version: Text,
  ...
  Args:
    ...
    ai_platform_serving_args: Dictionary containing arguments for pushing to
      AIP. For the full set of parameters supported, refer to
  ...
  """
  absl.logging.info(
      'Deploying to model with version {} to AI Platform for prediction: {}'
  ...
  while True:
    ...
    if 'error' in deploy_status:
      ...
      raise RuntimeError(
          'Failed to deploy model to AI Platform for prediction: {}'.format(
              deploy_status['error']))
